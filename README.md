...
bootstrap，然后优先恢复 Raycast 和 kitty。

0. 旧机器迁移前刷新备份

先刷新 Raycast 和 kitty 相关备份。Raycast 的 `.rayconfig` 需要在 Raycast 应用内
执行 Export Settings，并保存到 `~/.config/backups/raycast/`。

Homebrew Bundle 和 Mac App Store 应用恢复清单后续再整理；不要为了迁移 Raycast 和
kitty 阻塞在 brew/mas 清单上。
...
6. 优先恢复 Raycast 和 kitty

先安装常用入口工具，切回熟悉的启动器和终端环境，再继续后续迁移。
