...
just --justfile ~/.config/justfile system-config-macos-preferences-backup-menu-darwin
find ~/.config/backups/macos/preferences -maxdepth 1 -name '*.plist' -type f | sort

yadm add ~/.config/backups/Brewfile \
         ~/.config/backups/mas \
         ~/.config/backups/npm-skills \
         ~/.config/backups/macos/preferences
...
```bash
# First migration: install core essentials without relying on fzf
just --justfile ~/.config/justfile core-util-essentials-darwin
```

之后需要选择性补装 core 工具时再使用菜单：
...
just --justfile ~/.config/justfile system-config-macos-preferences-restore-menu-darwin
```
...
AI 相关配置只迁移上面列出的 config 和 key 文件。
